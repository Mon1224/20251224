...
# Config
# =========================
# ✅ 改成“本地 base 模型目录”，不要写 "Qwen/Qwen3-8B"
MODEL_ID = "/root/lanyun-tmp/Qwen3-8B/Qwen3-8B"   # <<< 按你云端实际路径改
LORA_DIR = "/root/LLaMA-Factory/acos_lora_output" # <<< 你的LoRA输出目录（adapter目录）

DEV_JSON = "/root/data/json_data/dev.json"         # <<< 按云端路径改
OUT_JSONL = "/root/data/json_data/pred_dev_lora.jsonl"
...
def main():
    ...
    model = PeftModel.from_pretrained(base_model, LORA_DIR)  # ✅ NEW
    model.eval()
    # ================================================
    ...
    with open(OUT_JSONL, "a", encoding="utf-8") as fout:
        for item in dev_data:
            ...
            # main
            messages = build_messages_main(review, few_shots=None)
